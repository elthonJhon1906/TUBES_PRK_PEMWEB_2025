...
final class ServiceCategory
{
    ...
    {
        $stmt = $this->pdo->query(
            'SELECT id, code, name, description, is_active, created_at FROM service_categories ORDER BY created_at DESC'
        );
    ...
    public function find(int $id): ?array
    {
        $stmt = $this->pdo->prepare(
            'SELECT id, code, name, description, is_active, created_at FROM service_categories WHERE id = :id'
        );
        $stmt->execute(['id' => $id]);
        $row = $stmt->fetch();
